...
async function run() {
    ...
    const prFiles = await octokit.request('GET /repos/{owner}/{repo}/pulls/{prNumber}/files', {
        owner: context.repo.owner,
    ...
    const filesToLint = prFiles.data
        .filter(f => constants_1.EXTENSIONS_TO_LINT.has(path.extname(f.filename)) && f.status !== "removed")
        .map(f => f.filename);
